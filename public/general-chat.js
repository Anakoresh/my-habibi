...
async function sendMessage(message) {
  if (!message.trim() || !userLoaded) {
    console.warn("User not loaded yet or empty message.");
    return;
  }
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  setTimeout(() => {
    if (!isFirstVisit && firstUnreadMessage) {
      addUnreadDivider(firstUnreadMessage);
      firstUnreadMessage.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
...
const updateMembersCount = async () => {
  ...
  try {
    ...
    console.error("Error by receiving members:", error);
    document.querySelector(".members-number").textContent = "Error";
  }
};
